...
const light_img = document.querySelector('.lightbox__image');

function renderUsers(arrGallery, ref) {
  const markup = arrGallery.reduce((acc, el) => {
    const li = `<li class="gallery__item"><img class="gallery__image" src="${el.preview}" data-source="${el.original}" alt="${el.description}"></li>`;
    return (acc += li);
  }, '');
...
lightbox.addEventListener('click', function (event) {
  if (event.target.nodeName === 'BUTTON' || event.target === content) {
